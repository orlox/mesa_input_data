...
! ***********************************************************************
      ...
      subroutine extras_controls(id, ierr)
         ...
         ! this will be turned to false when star is close to thermal equilibrium
         ! if true, then ignore mass loss
         s% lxtra1 = .true.
      ...
      integer function extras_finish_step(id, id_extra)
         ...
         write(*,*) "vrot/initial vrot is", s% v_rot_avg_surf * 1d-5, "/", s% job% new_surface_rotation_v

         if(.not. s% lxtra1 .and. abs(s% xtra1_old - s% center_h1) > 0.0005) then
         ...
         if (s% lxtra1 .and. &
            ...
            write(*,*) "Engage mass loss!"
            ! save ZAMS profiles
            call star_write_profile_info(s% id, "LOGS/prof_1ZAMS.data", s% id, ierr)
            if (ierr /= 0) return ! failure in profile
            ! relax rotation again, to ensure initial rotation is close to the input one
            write(*,*) "re-relaxing vrot"
            ...
            if (ierr /= 0) return ! failure in relax uniform omega
         end if
