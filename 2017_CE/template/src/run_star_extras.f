...
! ***********************************************************************
      ...
      integer function extras_finish_step(id, id_extra)
         ...
         if (s% center_h1 > 1d-6) then
            tams_radius = max(tams_radius, s% r(1))
            do while(log10_cr(s% r(1)/Rsun)*100>next_log_R)
               next_log_R = next_log_R + dlog_R
            end do
         else
            if (log10_cr(s% r(1)/Rsun)*100>next_log_R) then
               write(fname, fmt="(a12,i0.3,a4)") 'models/model', next_log_R, '.mod'
               write(*,*) "Writing a model ", fname
         ...
         if(abs(s% xtra25_old - s% center_h1) > 0.0005) then
          ...
          else if(abs(s% xtra26_old - s% center_he4) > 0.0005) then
             s% dt_next = min(s% dt_next, s% dt * s% min_timestep_factor)
             write(*,*) "reducing dt due to large change in central helium"
          else if(abs(s% xtra27_old - s% center_c12) > 0.0005) then
             s% dt_next = min(s% dt_next, s% dt * s% min_timestep_factor)
         ...
         if (s% center_he4 < 1d-2 .and. s% center_c12 < 1d-2) then
            write(*,*) "Terminating, close to carbon depletion"
